%%%锟斤拷T0锟斤拷锟姐到T1
function U=Finalpdf1(eps,x0,T1,T2)
alpha=1.75;
a1=-2.5;   b1=2.5;                        %左右区间端点
eps =( eps^alpha)*(2/(b1-a1))^alpha;      %levy参数
sigma=@(x)       (sqrt(0.9135*x.^2+1.9535*x+1.1450)).^2*(2/(b1-a1))^2;             %布朗参数
f= @(x,t)        (-0.9998*x.^3+3.9464*x)*2/(b1-a1);   %drift项
rr=1;             %right
rl=1;             %left
...
% coefficient of U_j
b(2:Jt-1) =( -2*((sigma(X(J+3:2*J+L-1))./2)/h^2)- c1*(1./(x(J+3:2*J+L-1)+rl).^alpha+1./(rr-x(J+3:2*J+L-1)).^alpha))';
% one-sided diff near boundaries
b(1)  = 2*((sigma(X(J+2))./2)/h^2)- c1*(1/(rl+x(J+2))^alpha+1/(rr-x(J+2))^alpha); 
b(Jt) = 2*((sigma(X(2*J+L))./2)/h^2)- c1*(1/(rl+x(2*J+L))^alpha+1/(rr-x(2*J+L))^alpha); % one-sided diff
...
a= ((sigma(X(J+1:2*J+L-1))./2)/h^2)'.*ones(Jt,1);  % coefficient of U_(j-1)
c= ((sigma(X(J+3:2*J+L+1))./2)/h^2)'.*ones(Jt,1);  % coefficient of U_(j+1) 
c(1)  = -5*((sigma(X(J+3))./2)/h^2); % one-sided diff
a(Jt) = -5*((sigma(X(2*J+L-1))./2)/h^2); % one-sided diff
vp2 = zeros(Jt,1); vp2(3) = 4*((sigma(X(J+4))./2)/h^2);  % one-sided diff
vp3 = zeros(Jt,1); vp3(4) =  -((sigma(X(J+5))./2)/h^2); % one-sided diff 
vm2 = zeros(Jt,1); vm2(Jt-2) = 4*((sigma(X(2*J+L-2))./2)/h^2);  % one-sided diff
vm3 = zeros(Jt,1); vm3(Jt-3) =  -((sigma(X(2*J+L-3))./2)/h^2); % one-sided diff 
 % integral part
 for j=-L+1:J-1
